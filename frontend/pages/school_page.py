    ...
    with tgb.part(class_name="main"):
        with tgb.part(class_name="container"):
            ...
            with tgb.part(class_name="card description-card"):
                tgb.text(
                    "Denna dashboard visar statistik per skola inom yrkeshögskolan (YH) i Sverige. "
                    "Här kan du se antal sökta och beviljade utbildningar samt beviljandegrad per anordnare. "
                    "Visualiseringar och tabeller ger en tydlig överblick över vilka skolor som har flest godkända ansökningar.",
                    mode="md",
                )
            ...
            with tgb.layout("1fr 1fr 1fr", gap="1.5rem", class_name="summary-cards"):
                ...
                with tgb.part(class_name="card card-student"):
                    ...
                    tgb.text("#### {{antal_ansökningar} - {antal_beviljade}}", mode="md")
...
def calculate_state(state):
    ...
    antal_ansökningar = len(filtered)
    antal_beviljade = len(filtered[filtered["Status"] == "Beviljad"])
    antal_avslag = len(filtered[filtered["Status"] == "Avslag"])
    beviljandegrad = (
        round(antal_beviljade / antal_ansökningar * 100, 2)
        if antal_ansökningar > 0
        else 0
